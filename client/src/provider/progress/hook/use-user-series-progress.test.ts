...
describe('useUserSeriesProgress', () => {
  ...
  it('calculates the average series progress as a rounded percentage', () => {
    stubProgress([
      {
    ...
    stubBooks([[makeBook('b1', 1), makeBook('b2', 2)], false, false, undefined]);
    const { result } = renderHook(() => useUserSeriesProgress('alice', 'TestSeries'));
    // avg = (0.5 + 1.0) / 2 = 0.75; 0.75 * 100 = 75
    expect(result.current).toEqual([75, false, false, undefined]);
  });

  it('treats missing progress for a book as 0 when calculating the average', () => {
    stubProgress([
      { 'b1': { document: 'b1', percentage: 1.0 } },
      false,
      false,
      undefined,
    ]);
    stubBooks([[makeBook('b1', 1), makeBook('b2', 2)], false, false, undefined]);
    const { result } = renderHook(() => useUserSeriesProgress('alice', 'TestSeries'));
    // avg = (1.0 + 0) / 2 = 0.5; 0.5 * 100 = 50
    expect(result.current).toEqual([50, false, false, undefined]);
  });
  ...
  it('rounds the percentage to the nearest integer', () => {
    ...
    stubBooks([
      [makeBook('b1', 1), makeBook('b2', 2), makeBook('b3', 3)],
      false,
      false,
      undefined,
    ]);
    const { result } = renderHook(() => useUserSeriesProgress('alice', 'TestSeries'));
    expect(result.current[0]).toBe(Math.round((1 / 3) * 100));
  });
});
