...
export const useUserSeriesProgress = (username: string | undefined, seriesName: string): UseUserSeriesProgress => {
  ...
  }, [progressList, loading, error, errorMessage, userProgressList, seriesBookList],
  );
};

const calculateSeriesProgressPercent = (books: Book[], progressMap: UserProgressList): number | undefined => {
  if (!books.some(book => progressMap[book.id])) {
    return undefined;
  }
  const avg = books.reduce((sum, book) => {
